...
async function loadReadBooks() {
    try {
        ...
        // Show books that have 'read' in their statuses array
        allReadBooks = userBooks.filter(b => {
            if (b.statuses && Array.isArray(b.statuses)) {
                return b.statuses.includes('read');
            }
            return b.status === 'read'; // Backward compatibility
        });
        
        displayBooks(allReadBooks);
    } catch (error) {
...
document.getElementById('searchInput').addEventListener('input', (e) => {
    ...
    const filtered = allReadBooks.filter(book => 
