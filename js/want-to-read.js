...
let allWishlistBooks = [];
let currentBook = null;

async function loadWishlist() {
    try {
        const userBooks = await API.getUserBooks(currentUser.id);
        // Show books that have 'want' in their statuses array
        allWishlistBooks = userBooks.filter(b => {
            if (b.statuses && Array.isArray(b.statuses)) {
                return b.statuses.includes('want');
            }
            return b.status === 'want'; // Backward compatibility
        });
        
        displayBooks(allWishlistBooks);
    } catch (error) {
...
document.getElementById('searchInput').addEventListener('input', (e) => {
    ...
    const filtered = allWishlistBooks.filter(book => 
